...
const PopularRentalCardSettings: React.FC<SectionEditorProps> = ({
  settings,
  handleSettingChange
}) => {
  const objectPath="sections.rentals.card";
  ...
  return (
    <div className="space-y-[.3vh]">
      ...
      <AnimatePresence>
        {activePanel === "Text" && (
          <SlidingPanel onClose={closePanel} isOpen={true} title="Card Text Settings">
            <div className="space-y-[.3vh] ">
                <FirstOrderSubSettingsContainer
                    name="Rental Name"
                    onClick={() => setActivePanel("rental_name")}
                />
                <FirstOrderSubSettingsContainer
                    name="Rental Description"
                    onClick={() => setActivePanel("rental_description")}
                />
                <FirstOrderSubSettingsContainer
                    name="Price"
        ...
        {activePanel === "rental_name" && (
          <SlidingPanel onClose={() => setActivePanel("text")} isOpen={true} title="Rental Name Settings">
        ...
        {activePanel === "rental_description" && (
          <SlidingPanel onClose={() => setActivePanel("text")} isOpen={true} title="Rental Description Settings">
              <UnderlinedTextSettings
        ...
        {activePanel === "price" && (
          <SlidingPanel onClose={() => setActivePanel("text")} isOpen={true} title="Price Settings">
              <UnderlinedTextSettings
                objectPath={`${objectPath}.textAndButton.text.price`}
                settings={settings}
                handleSettingChange={handleSettingChange}
                responsiveSize
                allowInput
        ...
        {activePanel === "duration" && (
          <SlidingPanel onClose={() => setActivePanel("text")} isOpen={true} title="Duration Settings">
              <UnderlinedTextSettings
                objectPath={`${objectPath}.textAndButton.text.duration`}
                settings={settings}
                handleSettingChange={handleSettingChange}
                responsiveSize
                allowInput
...
export default PopularRentalCardSettings