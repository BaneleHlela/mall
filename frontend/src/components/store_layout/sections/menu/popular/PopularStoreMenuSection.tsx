...
import PopularStoreMenuCard from '../shared_store_menu_components/PopularStoreMenuCard';

const PopularStoreMenuSection = () => {
  const settings = useAppSelector(state => state.layoutSettings.sections.menu) || mockLayout.sections.menu;
  ...
  return (
    ...
    >
      ...
      {store?.categories?.products && settings.categorySelector.show && (
        <div className="w-full pb-4 lg:pt-5 flex flex-row justify-center">
          <CategorySelector
      ...
      {isHorizontal ? (
        settings.categoryDivider?.show && store?.categories?.products ? (
          ...
          // Horizontal Swiper without Category Dividers
          <div
            style={{
              '--swiper-pagination-color': colors[settings.text.heading.color as keyof typeof colors],
              '--swiper-pagination-bullet-inactive-color': colors[settings.text.heading.color as keyof typeof colors] + "55",
        ...
        // Vertical Grid without Category Dividers
        <div
          className={`w-full flex flex-row ${
            settings.grid.container.background.position === 'center'
              ? 'justify-center'
...
export default PopularStoreMenuSection;
