...
const StoreCard: React.FC<StoreCardProps> = ({ store, allowShadow, onFavoriteClick, mini = false }) => {
  ...
  return (
    <div
        onClick={handleClick}
        onDoubleClick={isMobile ? undefined : handleFavoriteClick}
        className={isMobile ? `group relative w-full max-w-[200px] cursor-pointer transition-all duration-300 ease-out hover:scale-[1.02] hover:-translate-y-1 bg-white rounded-2xl shadow-md hover:shadow-2xl transition-shadow duration-300` : `relative aspect-4/3 border-1 border-gray-200  overflow-hidden ${allowShadow && "shadow-[0px_0px_12px_0px_rgba(0,_0,_0,_0.1)]"} absolute inset-0 bg-white/10 backdrop-blur-md transition-transform transform hover:-translate-y-1 z-0 rounded-[1.2vh] lg:rounded-[1.7vh] overflow-hidden`}
    >
      
      {/* Image */}
      <div className={`relative ${isMobile ? 'aspect-[4/3] overflow-hidden' : `${mini ? "h-[65%]" : "h-[72%]"} `}`}>
        <img src={hasWebsite ? store.thumbnails.storeCard : storeNotReadyThumbnail } alt="store-thumbnail" className={`w-full h-full object-cover ${isMobile ? 'transition-transform duration-500 group-hover:scale-110' : '-z-1'}`} />
        {isMobile && <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent" />}
        ...
        {/* Distance */}
        <p
          className={isMobile ? 'absolute bottom-2 left-2 bg-white/90 backdrop-blur-sm px-2.5 py-1 rounded-full text-xs font-medium text-gray-700 shadow-lg' : `absolute left-[1%] bg-white ${mini ? 'text-[1.3vh] rounded-xs bottom-0' : 'text-[1.8vh] rounded bottom-[2%] '} text-center text-black min-w-[15%] px-[1%]`}>{distanceText}</p>
        {/* Store State */}
        {!isMobile && store.storeState && (
      ...
      <div className={isMobile ? 'p-3 bg-white' : `flex flex-col justify-center ${mini ? "h-[40%]" : "h-[28%]"} shadow-md z-1 `}>
        {/* Store name */}
        <h3 className={isMobile ? 'font-bold text-gray-900 text-base line-clamp-1 mb-1 group-hover:text-indigo-600 transition-colors' : `text-center font-semibold text-gray-900 ${mini ? "text-[1.5vh]": "text-[90%]"} line-clamp-1`}>{store.nickname || store.name}</h3>
        {/* Details */}
        {isMobile ? (
          <>
            {store.slogan && (
